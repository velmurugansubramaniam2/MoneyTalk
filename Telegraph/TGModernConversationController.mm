...
//        _signUpController = [TCSMTSignUpController new];
//        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleShowConfirmationSMSNotification:) name:TCSNotificationShowConfirmationSMS object:nil];
    }
...
- (void)inputPanelRequestedAttachmentsMenu:(TGModernConversationInputTextPanel *)__unused inputTextPanel
{
    if (iosMajorVersion() >= 7 && !TGIsPad())
    {
        ...
        TGAttachmentSheetButtonItemView *transferItem = [[TGAttachmentSheetButtonItemView alloc] initWithTitle:TGLocalized(@"Transfer.Perform") pressed:^
                                                         {
                                                             __strong TGModernConversationController *strongSelf = weakSelf;
                                                             if (strongSelf != nil)
                                                             {
                                                                 [strongSelf.view endEditing:true];
                                                                 [strongSelf->_attachmentSheetWindow dismissAnimated:true completion:nil];
                                                                 [strongSelf openTransferViewControllerWithReceiver:nil];
                                                             }
                                                         }];
        [transferItem setBold:YES];
    ...
    else
    {
        NSMutableArray *actions = [[NSMutableArray alloc] initWithArray:@[
            [[TGActionSheetAction alloc] initWithTitle:TGLocalized(@"Transfer.Perform") action:@"performTransfer"],
        ...
        TGActionSheet *actionSheet = [[TGActionSheet alloc] initWithTitle:nil actions:actions actionBlock:^(TGModernConversationController *controller, NSString *action)
        {
            if (![action isEqualToString:@"cancel"])
            {
                ...
                if ([action isEqualToString:@"performTransfer"])
                    [controller openTransferViewControllerWithReceiver:nil];
    ...
    [self.navigationController dismissViewControllerAnimated:YES completion:^
     {
         ...
         if (strongSelf)
         {
             [strongSelf openTransferViewControllerWithReceiver:receiver];
         }
     }];
}

- (void)openTransferViewControllerWithReceiver:(TGUser *)receiver
{
    ...
    NSArray *receiversArray = set.allObjects;
    
    if (!receiver && receiversArray.count == 1)
    {
        int32_t uid = [[receiversArray firstObject] intValue];
        TGUser *user = [TGDatabaseInstance() loadUser:uid];
        receiver = user;
    }
    
    UIViewController *presentingViewController = TGIsPad() ? self : self.navigationController;
    UIViewController *presentedViewController = nil;
    
    if (!receiver && conversation.chatParticipantCount > 1)
    {
        presentedViewController = [[TCSMTReceiverSelectionViewController alloc] initWithConversationId:conversationId];
        [(TCSMTReceiverSelectionViewController *)presentedViewController setDelegate:self];
    }
    else
    {
        presentedViewController = [[TCSMTTransferViewController alloc] initWithConversationId:conversationId];
        [(TCSMTTransferViewController *)presentedViewController setDelegate:self];
        [(TCSMTTransferViewController *)presentedViewController setSelectedReceiver:receiver];
    }
    ...
    [[TCSMTLoginController sharedInstance] showViewController:navigationController fromViewController:presentingViewController];
}
