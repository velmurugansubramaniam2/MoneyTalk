...
@interface TGModernConversationInputTextPanel () <HPGrowingTextViewDelegate, TGModernConversationInputMicButtonDelegate>
{
    ...
    TGModernButton *_moneyTalkButton;
...
- (instancetype)initWithFrame:(CGRect)frame accessoryView:(UIView *)panelAccessoryView
{
    ...
    {   
        ...
        dispatch_once(&onceToken, ^
        {
            ...
            attachImage = [UIImage imageNamed:@"rouble_icon"];//менять картинку тут
        });
...
//        
//        _moneyTalkButton = [[TGModernButton alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 28.0f, 28.0f)];
//        [_moneyTalkButton.layer setCornerRadius:3.0f];
//        [_moneyTalkButton addTarget:self action:@selector(moneyTalkButtonPressed) forControlEvents:UIControlEventTouchUpInside];
//        [_moneyTalkButton setImage:[UIImage imageNamed:@"shaurma"] forState:UIControlStateNormal];
...
//    [commands addObject:_moneyTalkButton];
...
- (void)moneyTalkButtonPressed
{
    id<TGModernConversationInputTextPanelDelegate> delegate = (id<TGModernConversationInputTextPanelDelegate>)self.delegate;
    if ([delegate respondsToSelector:@selector(inputPanelMoneyTalkButtonPressed:)])
    {
        [delegate inputPanelMoneyTalkButtonPressed:self];
    }
}
