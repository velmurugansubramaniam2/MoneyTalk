...
- (BOOL)openURL:(NSURL *)url forceNative:(BOOL)__unused forceNative
{
    ...
    NSURL *mtOfertaUrl = [NSURL URLWithString:[TCSMTConfigManager sharedInstance].config.ofertaUrl];
    NSURL *mtTransferConditionsUrl = [NSURL URLWithString:[TCSMTConfigManager sharedInstance].config.transferConditionsUrl];
    
    if (iosMajorVersion() >= 9 && ([url.scheme isEqual:@"http"] || [url.scheme isEqual:@"https"]) && ![url isEqual:mtOfertaUrl] && ![url isEqual:mtTransferConditionsUrl]) {
